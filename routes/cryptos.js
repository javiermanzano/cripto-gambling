...
router.get('/', (req, res, next) => {
  axios({
    ...
    if (Math.random() > 0.5) {
      response.data.data[1027].quotes.USD.price = response.data.data[1027].quotes.USD.price + deviation;
    } else {
      response.data.data[1027].quotes.USD.price = response.data.data[1027].quotes.USD.price - deviation;
    }
    const LTC = response.data.data[2];
    if (Math.random() > 0.5) {
      response.data.data[2].quotes.USD.price = response.data.data[2].quotes.USD.price + deviation;
    } else {
      response.data.data[2].quotes.USD.price = response.data.data[2].quotes.USD.price - deviation;
    }
    const TRX = response.data.data[1958];
    if (Math.random() > 0.5) {
      response.data.data[1958].quotes.USD.price = response.data.data[1958].quotes.USD.price + deviation;
    } else {
      response.data.data[1958].quotes.USD.price = response.data.data[1958].quotes.USD.price - deviation;
    }
    res.json([
